...
function addMarker(place)
{
    ...
    marker.addListener("click", function(){
        ...
        $.getJSON("/articles",parameters, function(data){
            var content = "<ul>"
            for (var i=0, max=data.length; i<max && i<6 ; i++)
            {
    ...
    markers.push(marker);
    var check = marker.getMap();
    if (check == null || typeof check =="undefined"){
        alert("marker undefined" + place["lattitude"]);
    }
}
...
{
    setMapOnAll(null); // not defined, need to custom
    clearMarkers();
    markers = [];
}
